    ...
    class InferenceHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            page = "<form enctype=\"multipart/form-data\" method=\"post\">" \
                   ...
                   "Begin flag: <input type=\"text\" name=\"begin\" value=\"1\"><br>" \
        ...
        def do_POST(self):
            ...
            # Handle both file input (from C++ client) and text input (from web form)
            begin_field = form.getfirst('begin', '1')
            if hasattr(form['begin'], 'file') and form['begin'].file:
                begin_flag = int(form['begin'].file.read())
            else:
                begin_flag = int(begin_field)
            print("begin_flag:%d\n" % begin_flag)
